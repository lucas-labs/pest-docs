...
Open your browser or use a tool like curl to make a GET request:

```bash
$ curl http://127.0.0.1:8000/haunted-houses
```

You should observe a JSON list of haunted houses. Great success!
...
receive a` house_id` argument with the value `1`. Using our service instance, we retrieve the haunted house by
